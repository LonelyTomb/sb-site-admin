...
export default {
  ...
  methods: {
    ...mapActions({
      ...
      getRealtors: 'realtor/realtors',
      exportRealtor: 'realtor/export',
    }),
    ...
    async exportData() {
      const loader = this.$loading.show()
      try {
        const res = await this.exportRealtor()
        this.saveAsCSV('realtors', res)
        loader.hide()
      } catch (e) {
        loader.hide()
    ...
    async searchData(val) {
      const loader = this.$loading.show()
      try {
        await this.getRealtors({ search: val })
        loader.hide()
      } catch (e) {
        loader.hide()
        await this.$formatError(e)
